...
use crate::agent::memory_palace::types::MemoryPalaceSnapshot;
...
pub(crate) fn build_memory_snapshot(
    ...
    let (update_batch, pruned_edges) = prune_memory_update_batch(batch, 0.5);
    let snapshot = MemoryPalaceSnapshot {
        thread_id: thread_id.map(str::to_string),
        task_id: task_id.map(str::to_string),
        summary: format!(
            "memory snapshot with {} nodes and {} edges",
