# Requirements Archive: v3.0 The Intelligence Layer
...
- [x] **EPIS-08**: Session headers with auto-generated summary and tags on session end
...
- [x] **HAND-01**: HandoffBroker matches tasks to specialist profiles by capability tags (proficiency levels: expert, advanced, competent, familiar)
- [x] **HAND-02**: Context bundles carry typed references: memory refs, episodic refs, document refs, partial outputs
- [x] **HAND-03**: Context bundles are summarized with strict token ceiling (not forwarded raw) to prevent exponential growth
- [x] **HAND-04**: Escalation chains with structured triggers (ConfidenceBelow, ToolFails, TimeExceeds) and actions (HandBack, RetryWithNewContext, EscalateTo, AbortWithReport)
- [x] **HAND-05**: Orchestrator validates specialist output against acceptance criteria before accepting
- [x] **HAND-06**: Every handoff logged to WORM audit trail (from, to, task, outcome, duration, confidence, audit_hash)
- [x] **HAND-07**: Default specialist profiles ship out of the box (researcher, backend-developer, frontend-developer, reviewer, generalist)
- [x] **HAND-08**: Handoff depth limit (max 3 hops) to prevent handoff loops, then escalate to operator
- [x] **HAND-09**: HandoffBroker layers on existing spawn_subagent primitive (not a separate orchestration engine)

### Divergent Subagents

- [x] **DIVR-01**: Parallel interpretation mode where multiple framings work the same problem simultaneously
- [x] **DIVR-02**: Disagreement between framings is surfaced as the valuable output (tensions, not consensus)
- [x] **DIVR-03**: Mediator synthesizes tensions into a recommendation that acknowledges tradeoffs

### Uncertainty Quantification

- [x] **UNCR-01**: Planning confidence: each goal plan step rated HIGH/MEDIUM/LOW with evidence and dissent
- [x] **UNCR-02**: Tool-call confidence: pre-execution warnings with blast-radius uncertainty before policy-flagged commands
- [x] **UNCR-03**: Output confidence: research results labeled by source authority (official/community/unknown) and freshness
- [x] **UNCR-04**: Domain-specific escalation: Safety/Reliability domains block on LOW, Research/Business surface without blocking
- [x] **UNCR-05**: Operator preferences: configurable thresholds per domain in agent config
- [x] **UNCR-06**: Confidence derives from hybrid signals (structural: tool success rates, episodic familiarity, blast radius + verbal LLM self-assessment), not LLM alone
- [x] **UNCR-07**: Calibration feedback loop: operator corrections adjust confidence model over time
- [x] **UNCR-08**: If all plan steps are HIGH -> proceed autonomously. Any MEDIUM -> inform operator. Any LOW -> require approval.
...
- [x] **AUTH-01**: Significant outputs attribute contributions: what came from operator input, what from agent synthesis, what's joint
- [x] **AUTH-02**: Attribution is metadata on the output, not inline commentary (not disruptive to reading flow)

### Cost & Token Accounting

- [x] **COST-01**: Per-goal token counts (prompt + completion) tracked on every LLM API call
- [x] **COST-02**: Per-session and cumulative cost estimates using provider rate cards
- [x] **COST-03**: Budget alerts when spending exceeds operator-defined threshold
- [x] **COST-04**: Cost data persisted in goal_run metadata, queryable via observability

### Autonomy Dial

- [x] **AUTO-01**: Per-goal autonomy level setting: autonomous / aware / supervised
- [x] **AUTO-02**: Autonomous: agent proceeds, operator sees final report only
- [x] **AUTO-03**: Aware: agent reports on milestones (sub-task completions, handoffs)
- [x] **AUTO-04**: Supervised: agent reports on every significant step and waits for acknowledgment

### Explainability Mode

- [x] **EXPL-01**: On-demand reasoning log: why agent chose Plan A over Plan B with rejected alternatives
- [x] **EXPL-02**: "Why did you do that?" query returns causal trace for any past action
- [x] **EXPL-03**: Rejected alternatives and decision points stored alongside chosen plan in goal_run metadata
...
|-------------|-------|--------|
| EPIS-01 | Phase 1 | Complete |
| EPIS-02 | Phase 1 | Complete |
| EPIS-03 | Phase 1 | Complete |
| EPIS-04 | Phase 1 | Complete |
| EPIS-05 | Phase 1 | Complete |
| EPIS-06 | Phase 1 | Complete |
| EPIS-07 | Phase 1 | Complete |
| EPIS-08 | Phase 9 | Complete |
| EPIS-09 | Phase 1 | Complete |
| EPIS-10 | Phase 1 | Complete |
| EPIS-11 | Phase 1 | Complete |
...
| EMBD-01 | Phase 9 | Complete |
| EMBD-02 | Phase 9 | Complete |
| EMBD-03 | Phase 9 | Complete |
| EMBD-04 | Phase 9 | Complete |
| UNCR-01 | Phase 2 | Complete |
| UNCR-02 | Phase 4 | Complete |
| UNCR-03 | Phase 4 | Complete |
| UNCR-04 | Phase 2 | Complete |
| UNCR-05 | Phase 2 | Complete |
| UNCR-06 | Phase 2 | Complete |
| UNCR-07 | Phase 9 | Complete |
| UNCR-08 | Phase 7 | Complete |
| HAND-01 | Phase 3 | Complete |
| HAND-02 | Phase 3 | Complete |
| HAND-03 | Phase 3 | Complete |
| HAND-04 | Phase 9 | Complete |
| HAND-05 | Phase 9 | Complete |
| HAND-06 | Phase 3 | Complete |
| HAND-07 | Phase 3 | Complete |
| HAND-08 | Phase 3 | Complete |
| HAND-09 | Phase 3 | Complete |
| DIVR-01 | Phase 9 | Complete |
| DIVR-02 | Phase 8 | Complete |
| DIVR-03 | Phase 9 | Complete |
...
*Last updated: 2026-03-27 after milestone gap closure phase planning*
